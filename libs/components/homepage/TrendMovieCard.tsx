...
interface TrendMovieCardProps {
	movie: Movie;
	likeMovieHandler: any
}

const TrendMovieCard = (props: TrendMovieCardProps) => {
	...
	if (device === 'mobile') {
		return (
			<Stack className="trend-card-box" key={movie._id}>
				<Box
				...
				<Box component={'div'} className={'info'}>
					<strong  className={'title'}  onClick={() => pushDetailHandler(movie._id)} >
						{movie.movieName}
                         
					</strong>
					<p className={'desc'}>{movie.movieDesc ?? 'no description'}</p>
