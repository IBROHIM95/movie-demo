...
import MovieBigCard from '../../libs/components/common/PropertyBigCard';
...
import { MoviesInquiry } from '../../libs/types/movie/movie.input';
...
import { GET_COMMENTS, GET_MEMBER, GET_MOVIES } from '../../apollo/user/query';
...
const AgentDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	const [agentMovies, setAgentMovies] = useState<Movie[]>([]);
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted:(data: T) =>  {
			setAgentMovies(data?.getMovies?.list)
			setMovieTotal(data?.getMovies?.metaCounter[0]?.total ?? 0)
		}
	})
	...
	const moviePaginationChangeHandler = async (event: ChangeEvent<unknown>, value: number) => {
	...
	const likeMovieHandler = async (user: any, id:string) => {
		try{
		   if(!id) return;
		   if(!user._id) throw new Error(Messages.error2)
		   ...
		   await getMoviesRefetch({input: searchFilter})
				...
				<Stack className={'container'}>
					...
					<Stack className={'agent-home-list'}>
						<Stack className={'card-wrap'}>
							{agentMovies.map((movie: Movie) => {
								return (
									<div className={'wrap-main'} key={movie?._id}>
										<MovieBigCard movie={movie} key={movie?._id} />
									</div>
								);
							})}
						</Stack>
								...
								<>
									<Stack className="pagination-box">
										<Pagination
											...
											count={Math.ceil(movieTotal / searchFilter.limit) || 1}
											onChange={moviePaginationChangeHandler}
											shape="circular"
											color="primary"
										/>
									</Stack>
									<span>
										Total {movieTotal} movie{movieTotal > 1 ? 'ies' : 'y'} available
									</span>
								</>
							) : (
