...
import PropertyBigCard from '../../libs/components/common/MovieBigCard';
...
import { GET_COMMENTS, GET_MOVIES, GET_MOVIE } from '../../apollo/user/query';
import { T } from '../../libs/types/common';
import { Direction, Message } from '../../libs/enums/common.enum';
import { CREATE_COMMENT, LIKE_TARGET_MOVIE } from '../../apollo/user/mutation';
...
const MovieDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [nameMovies, setNameMovies] = useState<Movie[]>([]);
	const [commentInquiry, setCommentInquiry] = useState<CommentsInquiry>(initialComment);
	const [movieComments, setMovieComments] = useState<Comment[]>([]);
	const [commentTotal, setCommentTotal] = useState<number>(0);
	const [insertCommentData, setInsertCommentData] = useState<CommentInput>({
		commentGroup: CommentGroup.MOVIE,
		commentContent: '',
		commentRefId: '',
	});
	...
	const {
		...
		onCompleted:(data: T) =>  {
		 if(data?.getMovie) setMovie(data?.getMovie);
		 if(data?.getMovie) setSlideImage(data?.getMovie?.movieImages[0]);
		}
	})
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					countryList: movie?.movieCountry ? [movie?.movieCountry] : []
				}
			}},
			skip:!movieId && !movie,
     		notifyOnNetworkStatusChange: true,
		    onCompleted:(data: T) =>  {
			if (data?.getMovies?.list) setNameMovies(data?.getMovies?.list)
		}
	})
	...
	const {
		...
		variables: {
			...
			skip:!commentInquiry.search.commentRefId,
     		notifyOnNetworkStatusChange: true,
		    onCompleted:(data: T) =>  {
			if (data?.getComments?.list) setMovieComments(data?.getComments?.list)
	...
	const likeMovieHandler = async (user: T, id:string) => {
		try{
		   ...
		   await getMovieRefetch({input: id})
		   await getMoviesRefetch({input: {
			page: 1,
			limit: 4,
			sort: 'createdAt',
			direction: Direction.DESC,
			search: {
				locationList: [movie?.movieCountry],
			}
		}})
						...
						<Stack className={'property-info-config'}>
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									<Typography className={'title-main'}>{movie?.movieName}</Typography>
									<Stack className={'top-box'}>
										<Typography className={'city'}>{movie?.movieCountry}</Typography>
										...
										<Typography className={'date'}>{moment().diff(movie?.createdAt, 'days')} days ago</Typography>
									</Stack>
									<Stack className={'bottom-box'}>
										<Stack className="option">
											<img src="/img/icons/bed.svg" alt="" /> <Typography>{movie?.movieSeasons} season</Typography>
										</Stack>
										<Stack className="option">
											<img src="/img/icons/room.svg" alt="" /> <Typography>{movie?.movieDuration} hour</Typography>
										</Stack>
										<Stack className="option">
											<img src="/img/icons/expand.svg" alt="" /> <Typography>{movie?.movieYear} - year</Typography>
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'right-box'}>
									<Stack className="buttons">
										<Stack className="button-box">
											...
											<Typography>{movie?.movieViews}</Typography>
										</Stack>
										<Stack className="button-box">
											{movie?.meLiked && movie?.meLiked[0]?.myFavorite ? (
												...
												<FavoriteBorderIcon
													fontSize={'medium'}
													// @ts-ignore
													onClick={() => likePropertyHandler(user, movie?._id)}
												/>
											)}
											<Typography>{movie?.movieLikes}</Typography>
										</Stack>
									</Stack>
							...
							<Stack className={'images'}>
								...
								<Stack className={'sub-images'}>
									{movie?.movieImages.map((subImg: string) => {
								...
								<Stack className={'options-config'}>
									<Stack className={'option'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{movie?.movieSeasons}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											<Typography className={'title'}>Hour</Typography>
											<Typography className={'option-data'}>{movie?.movieDuration}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{moment(movie?.createdAt).format('YYYY')}</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											...
											<Typography className={'option-data'}>{movie?.movieYear} - year</Typography>
										</Stack>
									</Stack>
									<Stack className={'option'}>
										<Stack className={'svg-box'}>
										...
										<Stack className={'option-includes'}>
											<Typography className={'title'}>Movie Type</Typography>
											<Typography className={'option-data'}>{movie?.movieType}</Typography>
										</Stack>
									</Stack>
								</Stack>
								<Stack className={'prop-desc-config'}>
									<Stack className={'top'}>
										<Typography className={'title'}>Movie Description</Typography>
										<Typography className={'desc'}>{movie?.movieDesc ?? 'No Description!'}</Typography>
									</Stack>
									<Stack className={'bottom'}>
										<Typography className={'title'}>Movie Details</Typography>
										<Stack className={'info-box'}>
											<Stack className={'left'}>
												
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Movie year</Typography>
													<Typography className={'data'}>{movie?.movieYear} - year</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Season</Typography>
													<Typography className={'data'}>{movie?.movieSeasons}</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Hour</Typography>
													<Typography className={'data'}>{movie?.movieDuration}</Typography>
												</Box>
											</Stack>
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Movie Type</Typography>
													<Typography className={'data'}>{movie?.movieType}</Typography>
												</Box>
											</Stack>
										</Stack>
									</Stack>
								...
								<Stack className={'info-box'}>
									...
									<Stack className={'image-info'}>
										<img
											...
											src={
												movie?.memberData?.memberImage
													? `${REACT_APP_API_URL}/${movie?.memberData?.memberImage}`
													: '/img/profile/defaultUser.svg'
											}
										/>
										<Stack className={'name-phone-listings'}>
											<Link href={`/member?memberId=${movie?.memberData?._id}`}>
												<Typography className={'name'}>{movie?.memberData?.memberNick}</Typography>
											</Link>
											<Stack className={'phone-number'}>
												...
												<Typography className={'number'}>{movie?.memberData?.memberPhone}</Typography>
											</Stack>
							...
							<Stack className={'similar-properties-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>Movie name</Typography>
								...
								<Stack className={'cards-box'}>
									...
									>
										{nameMovies.map((movie: Movie) => {
											return (
												<SwiperSlide className={'similar-homes-slide'} key={movie?.movieName}>
													<PropertyBigCard 
...
export default withLayoutFull(MovieDetail);
