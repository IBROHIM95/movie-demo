...
const MovieList: NextPage = ({ initialInput, ...props }: any) => {
	const device = useDeviceDetect();
	const router = useRouter();
	const [searchFilter, setSearchFilter] = useState<MoviesInquiry>(
	...
	const {
		...
		onCompleted:(data: T) =>  {
			setMovies(data?.getMovies?.list)
			setTotal(data?.getMovies?.metaCounter[0]?.total)
		}
	})
							...
							<Stack className={'list-config'}>
								{movies?.length === 0 ? (
									...
									movies.map((movie: Movie) => {
										return <PropertyCard movie={movie} likeMovieHandler={likeMovieHandler} key={movie?._id} />;
									})
								)}
							</Stack>
							<Stack className="pagination-config">
								...
								{movies.length !== 0 && (
									<Stack className="total-result">
										<Typography>
											Total {total} movie{total > 1 ? 'ies' : 'y'} available
										</Typography>
									</Stack>
								)}
							</Stack>
...
export default withLayoutBasic(MovieList);
